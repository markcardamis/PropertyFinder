        ...
        if (propertyListings != null && propertyListings.length > 0) {
            ...
            for (int i = 0; i < propertyListings.length; i++) {
                try {
                    ...
                    if (propertyListings[i].address.toLowerCase().contains("lot")) { // lot numbers don't always get
                                                                                     // correct zone
                        propertyListings[i].zone = "NA";
                    } else if (!propertyListings[i].displayableAddress.toLowerCase().matches(".*\\d.*")) { // agents
                                                                                                           // don't
                                                                                                           // always
                                                                                                           // include
                                                                                                           // address
                        ...
                        address = UrlExtensionMethods.appendParameter(address, "id",
                                propertyListings[i].planningPortalPropId);
                        address = UrlExtensionMethods.appendParameter(address, "layers", "epi");

                        String responseJson = mServiceHelper.callHTTPService(address, HttpMethod.GET, "", false, "");
                        Gson gson = new Gson();
                        PlanningPortalZoneResponse[] planningPortalZoneResponse = gson.fromJson(responseJson,
                                PlanningPortalZoneResponse[].class);
                        for (int j = 0; j < planningPortalZoneResponse.length; j++) {
                            if (planningPortalZoneResponse[j].layerName.equals("Land Zoning")) {
                                ...
                                propertyListings[i].lgaName = planningPortalZoneResponse[j].results[0].LGA_Name;
                            } else if (planningPortalZoneResponse[j].layerName.equals("Floor Space Ratio (n:1)")) {
                                propertyListings[i].fsr = Float
                                        .valueOf(planningPortalZoneResponse[j].results[0].Floor_Space_Ratio);
                            } else if (planningPortalZoneResponse[j].layerName.equals("Minimum Lot Size")) {
                                propertyListings[i].minimumLotSize = planningPortalZoneResponse[j].results[0].title
                                        .replaceAll("\\D", "");
                            }
                        }
                    }
                ...
                System.out.println("PlanningPortalZone " + propertyListings[i].zone + " " + String.valueOf(i + 1) + "/"
                        + String.valueOf(propertyListings.length));
            }
        }
