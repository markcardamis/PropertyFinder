...
import {
  closeFilter,
  showFilter,
} from "../../../store/actions/filterModalAction";
import {
  closeLayersModal,
  showLayersModal,
...
import {
  closeSearchModal,
  showSearchModal,
} from "../../../store/actions/searchModalAction";
...
const MapGL = () => {
  ...
  const { searchModal, filterModal, saveModal, layers, mapMarker } =
  ...
  useEffect(() => {
    ...
    map.current.on("mousemove", "nsw-property", (e) =>
      handleHoverOnProperty(e)
    );
    map.current.on("mouseleave", "nsw-property", () =>
  ...
  return (
    <>
      <div
        ...
        id="map"
        style={{
          left: searchModal ? "45%" : 0,
          width: searchModal ? "55%" : "100%",
        }}
      >
        {!filterModal && !saveModal.showModal && (
          <FilterButtonGroup
            onListViewClick={() => dispatch(showSearchModal())}
            onMapViewClick={() => dispatch(closeSearchModal())}
            onFilterClick={() =>
              filterModal ? dispatch(closeFilter()) : dispatch(showFilter())
            }
            onLayersClick={() =>
              layers.showModal
                ? dispatch(closeLayersModal())
                : dispatch(showLayersModal())
            }
          />
        )}
